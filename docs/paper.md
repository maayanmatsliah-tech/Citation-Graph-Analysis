...
I set out to test whether ChatGPT, released in November 2022, increased the rate of *mutual citations* — the unusual case where two papers cite each other — by compressing peer discovery. Across roughly 1M papers from OpenAlex spanning 2020–2024, I find no evidence for the ChatGPT prediction: mutual citation rates declined steadily across the window, and the apparent step-change at ChatGPT's launch dissolves under a placebo test (in which 23 of 30 candidate break dates produce nominally "significant" Chow statistics, and removing a single outlier month kills the result). What I found instead is a clear, robust, previously-undocumented phenomenon: **the average paper now cites work from roughly 50% more distinct fields than it did three years earlier** (2.06 fields in 2021 to 3.11 in 2024), with a flat total reference budget. This broadening is monotone, persists when the analysis is restricted to fields with stable sample composition, and persists even within Computer Science taken alone. I argue it provides a quantitative mechanism for the decline in mutual citation rates over the same window: the same reference budget spread over more fields produces less within-field overlap, and mutual pair formation drops mechanically. The broadening trend is decelerating over time, not accelerating, so it is not a ChatGPT effect — it is a longer-running shift in how scientific literature is being cited.
...
I initially tested the following hypothesis:
...
- A **self-citation** is an edge from a paper to itself (an artifact in the raw data; excluded throughout).
...
Mutual citation rate is a deliberately sharp test of the discovery-compression hypothesis. Before ChatGPT, two contemporaneous papers citing each other required both authors to find each other's work during overlapping writing windows — typically by chance, through shared collaborators, or by happening to read the same recent venue. This is structurally rare, and the rarity is precisely what makes mutual rate a sensitive instrument: a tool that genuinely compresses peer discovery should be visible here even at modest adoption levels, because the baseline frequency is so low that small shifts are easy to detect. The mechanism is plausible — papers move through preprint, review, and revision stages where citations can still be added, so two authors who discover each other's recent work via ChatGPT have multiple opportunities to add reciprocal citations before publication. Mutual rate is thus a narrow but well-chosen surface of the broader hypothesis. The broader version — that ChatGPT changed citation behavior generally — would also naturally manifest in directional metrics (citation recency, breadth, time-to-first-citation), which I return to in §5.7.

## 3. Data

All data comes from [OpenAlex](https://openalex.org/), a public bibliographic database. I use two ingestion paths: the OpenAlex S3 snapshot and the OpenAlex API. Both write to the same `works` and `citations` tables in a single DuckDB database. The resulting dataset contains approximately 200,000 papers for each year from 2020 through 2024 — a deliberately flat denominator that simplifies year-over-year comparisons.

**Yearly vs monthly resolution.** Most analyses operate at yearly resolution, using the ~200k-per-year denominator directly. The monthly trajectory and Chow tests (§5.4–§5.5) needed a finer denominator. Rather than reingest publication dates for all ~1M papers, I backfilled full publication dates only for the ~16,000 papers that participate in any mutual pair across 2020–2024, and approximated monthly paper counts as `yearly_total / 12` — a uniform-within-year assumption on the denominator side, with real monthly resolution on the numerator side. This is an acceptable shortcut for *detecting* a sharp step-change at a specific month (the approximation error is roughly uniform within each year, not concentrated at any candidate break date), but it cannot resolve real seasonal swings in publication volume. The placebo-test result in §5.5 shows the monthly analysis was limited by other things first, so a real per-month denominator would not have changed the conclusion.

For each paper the database stores an ID, title, publication year, field, and — for the ~16,000 papers participating in any mutual pair across 2020–2024 — full publication date and paper type. For each citation it stores the citing and cited paper IDs.
...
A mutual pair is detected by joining the `citations` table to itself on the symmetric condition `a.citing_id = b.cited_id AND a.cited_id = b.citing_id`. A naive form of this query (a) counts every self-citation as a mutual pair, since the join matches `(X → X)` against itself, and (b) returns each true mutual pair `{X, Y}` twice. I resolve both with a single filter — `WHERE a.citing_id < a.cited_id` — which excludes self-cites and uniquely orders each unordered pair. All numbers in this paper use the corrected query.
...
For cross-field comparisons I use **difference-in-differences**, which compares the pre-to-post change in a treated group to the pre-to-post change in a control group. The estimator is valid causally only if the two groups were trending in parallel before treatment.

For citation breadth (the central finding) I compute, for each citing paper, the number of *distinct* OpenAlex fields that appear among its references, then average across all citing papers within a year cohort.

## 5. Results

I report the analyses in roughly the order they were performed. The early results (§5.1–5.3) constrained but did not falsify the original hypothesis; §5.4 tested it at monthly resolution and produced what initially looked like a positive result, which §5.5 showed was artifactual; §5.6 surfaced a methodological surprise (sample composition drift); §5.7 is the central positive finding; and §5.8 catalogs the candidate mechanisms ruled out along the way.
...
The yearly trajectory has only four data points and limited power against a small effect. I backfilled monthly publication dates for the ~16,000 mutual-pair papers and ran a Chow test at the ChatGPT cutoff (Dec 2022). The result looked striking:
...
This is the most coherent mechanistic explanation I found for the secular decline, and it is robust to the artifacts that defeated the other candidates I tested.

### 5.8 Mechanisms I ruled out
...
What remains is the citation-breadth mechanism in §5.7.

## 6. Discussion

The original ChatGPT hypothesis is not supported by this data. Mutual citation rates declined across the window, but the decline started before ChatGPT existed, follows a smooth trajectory across its launch, and fails its strongest sub-test (same-year pairs) too. The monthly-resolution test that initially looked positive turned out to be a placebo-level Chow artifact.

But the path to that null result surfaced a substantive finding. Citation patterns in the OpenAlex 2020–2024 record are broadening at a striking rate. The average paper cites work from roughly 50% more distinct fields than three years earlier, on the same flat reference budget. This is a real, large, monotone, multiply-robust phenomenon that almost no current bibliometric narrative about science publishing centers on, and it is — for now — the most parsimonious explanation I can identify for the substantial secular decline in mutual citations.
...
**The breadth expansion is not a ChatGPT effect.** The trend predates ChatGPT and is decelerating, not accelerating. If it has a single causal driver, that driver is something that has been operating for at least the 2020–2022 pre-ChatGPT period — candidates include search tools (Google Scholar, Semantic Scholar), the institutional rise of interdisciplinarity, and improving cross-field linkage in citation databases themselves. None of these are testable with the data here, and I make no claim about which is correct.

**The two metrics test the hypothesis at complementary scales.** Mutual citation rate is a sensitive test of the *specific* discovery-compression prediction: it would catch even a modest ChatGPT-driven uptick in contemporaneous awareness because the baseline is so low. Citation breadth tests the *broader* version of the claim — that ChatGPT changed citation behavior at all — and is the metric where the underlying shift in citation behavior is actually visible. The pair of results is informative: on the sharp test, no detectable ChatGPT effect; on the broad test, a substantial behavior change is unambiguously occurring, but it predates ChatGPT and is decelerating, so it cannot be attributed to it. Citation behavior is changing rapidly in the direction the original hypothesis predicted; the timing rules out ChatGPT as the cause.
...
- Chow tests on noisy monthly series with seasonality fire indiscriminately. In this data, 77% of candidate break dates were "significant" at p < 0.05; the ChatGPT date ranked 9th by F-statistic. Always run a placebo sweep.

## 7. Apparent findings the data cannot resolve

Two further patterns surfaced during the investigation that look like real signal but cannot be confirmed as behavior changes given the limits of this dataset. I gather them here rather than in Results because each is a partial finding contingent on data caveats that the project did not resolve.

**The "citation age increase" is mechanically forced by coverage.** Average age of cited papers rose from 0.09 years (2020 citing year) to 2.33 years (2024 citing year). Read naively, that looks like researchers shifting toward older work. It is not. For every citing year, 99.5–100% of matched citations land on papers in the dense 2020–2024 region of the works table, because pre-2020 coverage is sparse. The metric therefore mechanically measures how many years of the dense region are reachable backward from the citing year, which grows monotonically by construction. The trajectory observed (0.09 → 2.33 years) is exactly what this constraint produces regardless of behavior. I exclude this from the headline findings.

**The forward-citation collapse is real but unattributable.** Forward citations (cited_year > citing_year, only possible via preprint awareness or late edits) decline at roughly −18%/yr on citing-years 2021–2023. The preprint-deduplication mechanism I initially proposed for this is refuted in §5.8 — forward citations are 78% article→article and only 3% preprint-touching, and preprint-heavy fields decline *slower* not faster. The decline is therefore real but its cause is unknown after the obvious candidates were eliminated. It is the second-largest unexplained signal in the analysis, second only to the secular mutual-citation decline (which the breadth finding in §5.7 addresses).

**Per-field decline magnitudes cannot be ranked from this dataset.** §5.6 showed that the OpenAlex sample composition shifts substantially across years (Math −77%, Engineering +43%) — almost certainly an artifact of the dual S3/API ingestion pipeline rather than real publication-volume changes. Per-field rankings of decline rate are therefore contaminated. The aggregate decline survives the contamination (it does not depend on field labels), and the breadth finding survives in single-field and stable-field subsets. But per-field magnitudes — which field is declining "fastest" — cannot be trusted without single-pipeline reingestion.
...
- **The link between breadth and the mutual decline is mechanistic, not causal.** I argue the two are quantitatively compatible under simple independence assumptions, but have not directly demonstrated that the breadth expansion *causes* the mutual decline. A within-paper instrumental analysis would be needed to establish that.
...
1. **Replicate the breadth expansion in an independent citation graph.** Semantic Scholar and Crossref both expose citation data with different ingestion histories. If the rise appears in both, it is a real characteristic of scientific publishing in this period, not an OpenAlex artifact.
...
3. **Test whether the breadth-mutual link is causal at the paper level.** For each paper, do papers with higher breadth participate in fewer mutual pairs, controlling for field, year, and citation count? A within-paper test would confirm the mechanism implied by §5.7.
4. **Resolve the forward-citation collapse.** Forward (cited_year > citing_year) citations decline at roughly −18%/yr in this dataset, and the preprint-deduplication mechanism I initially hypothesized for this was refuted in §5.8. The cause is now unknown and is the second-largest unexplained signal in the analysis.

## 10. Conclusion

I tested a specific hypothesis about ChatGPT and mutual citations. The hypothesis is not supported by this data, at either yearly or monthly resolution; the apparent monthly break failed a placebo test. But the investigation led to a finding I did not expect and that is, on its own, larger and more substantively interesting than the original question: the average paper in the 2024 cohort cites work from roughly 50% more distinct fields than the average paper in the 2021 cohort, on the same flat reference budget, with a trend that is monotone, decelerating, and predates ChatGPT. This broadening provides a clean mechanistic account of the secular decline in mutual citation rates that I initially mistook for a ChatGPT effect, and it points to a longer-running and probably more consequential shift in how scientific work is being cited than any single technology release would explain.
